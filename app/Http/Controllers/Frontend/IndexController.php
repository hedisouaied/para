...
class IndexController extends Controller
{
    public function home()
    {
        ...
        $partenaires = Partenaire::where(['status' => 'active'])->orderBy('id', 'ASC')->get();

        $feedbacks = Feedback::where(['status' => 'active'])->orderBy('id', 'ASC')->get();

        $blogs = Blog::where(['status' => 'active'])->orderBy('id', 'DESC')->limit('2')->get();

        $hot_products = Product::where(['status' => 'active'])->where('discount' , '!=' , '0')->inRandomOrder()->get();
    ...
    public function tousproducts(Request $request)
    {
        ...
        return view('frontend.pages.product.tousproducts', compact(['products', 'route', 'title_page']));
    }
    ...
    public function grandcategoryDetail(Request $request,$slug)
    {
        ...
        if ($category) {
            ...
            $products = Product::where(['status' => 'active', 'grand_cat_id' => $category->id]);
            ...
            return view('frontend.pages.grandcategory-details', compact(['products', 'route', 'category', 'title_page']));
    ...
    public function categoryDetail(Request $request,$slug)
    {
        ...
        if ($category) {
            ...
            $products = Product::where(['status' => 'active', 'cat_id' => $category->id]);
            ...
            return view('frontend.pages.category-details', compact(['products', 'route', 'category', 'title_page']));
    ...
    public function souscategoryDetail(Request $request,$slug)
    {
        ...
        if ($category) {
            ...
            $products = Product::where(['status' => 'active', 'child_cat_id' => $category->id]);
            ...
            return view('frontend.pages.souscategory-details', compact(['products', 'route', 'category', 'title_page']));
    ...
    public function checkout(Request $request)
    {
        ...
        foreach ($cart_data as $keys => $values) {
            ...
            $array_prix[]=($product->price-(($product->discount)*$product->price)/100);
        ...
        if ($status) {
            ...
            return redirect()->route('home')->with('success', 'Commande a été passée avec succès');
        } else {
    ...
    public function compte()
    {
        ...
        if ((Auth::user())) {
            ...
            return view('frontend.auth.compte', compact(['user', 'title_page']));
        } else {
            return view('errors.404');
        }
    }
